...
def save_evaluation_files_v2(retriever, results, args):
    ...
    with open(resutls_per_rel_path, 'w', newline='') as rel_res_file, open(resutls_per_bk_path, 'w', newline='') as bk_res_file:
                    ...
                    if len(bk_value) == 0:
                        ...
                        for item in qrels_rel_data:
                            ...
                            if item["query_id"] in qid_list:
                                if query_id not in qrels_bk:
                                    qrels_bk[query_id] = {corpus_id: score}
                                else:
                                    qrels_bk[query_id][corpus_id] = score
                    
                                    
                        if len(qrels_bk) > 0:
                            ndcg, _map, recall, precision = retriever.evaluate(qrels_bk, results, k_values) #retriever.k_values
        ...
        for bk_name, bk_value in bk_all_data.items():
            ...
            print(f"len({bk_name}): {len(bk_value)}")
            if len(bk_value) == 0:
                print(f"{relation_id}_{bk_name} has no data.")
                eval_res = [f"{relation_id}_{bk_name}"] + [0]*12
            else:
                ...
                for item in qrels_rel_data:
                    ...
                    if item["query_id"] in qid_list:
                        if query_id not in qrels_bk_all:
                            qrels_bk_all[query_id] = {corpus_id: score}
                        else:
                            qrels_bk_all[query_id][corpus_id] = score
            
                            
                if len(qrels_bk_all) > 0:
                    ndcg, _map, recall, precision = retriever.evaluate(qrels_bk_all, results, k_values) #retriever.k_values
