...
def main():
    ...
    parser.add_argument('--knowledge_input_file', type=str)
    ...
    # Input test data  
    with open(args.test_queries_file, 'r') as in_queries:
        # query_data = {"religion": json.load(in_queries)}
        query_data = [json.loads(line) for line in in_queries]
        
    sample = pd.DataFrame(query_data)
